...
public class SurveyController extends HttpServlet implements Servlet {
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (command.equals("/survey/surveyForm.sv")) {
			RequestDispatcher dispatcher = request.getRequestDispatcher("/survey/surveyInsert.html");
			...
			action.execute(request);
			RequestDispatcher dispatcher = request.getRequestDispatcher("/survey/surveyForm.jsp");
			dispatcher.forward(request, response);
		}else if (command.equals("/survey/surveyOk.sv")) {
			SurveyOkAction action = new SurveyOkAction();
			action.execute(request);
			RequestDispatcher dispatcher = request.getRequestDispatcher("/survey/submitted.jsp");
			dispatcher.forward(request, response);
		}
	}
