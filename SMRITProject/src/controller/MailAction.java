...
public class MailAction {
	public void sendMail(MemberDTO dto) {
		...
		String content = dto.getUserName() + "님 안녕하세요. 가입을 축하합니다.<br/>"
				+"아래 링크를 누르셔야만 가입이 완료됩니다.<br/>" + "<a href='http://172.16.3.4:8080/SMRITProject/mem/memberJoinOk.mem?email="+dto.getUserEmail()+"&joinOk="+num+"' >가입 확인</a>"; 
...
@Override
		...
		try {
			...
			textMassage = dto.getUserName() + "님 가입을 환영합니다. 이메일로 접속하여 가입확인을 하여주시고 만약 본인이 아닌 경우에는 1588+1588로 문의 주세요.";
			ss.smsSend(dto.getUserPh1(), textMassage);
			
			
			
		} catch (Exception e) {
			textMassage = dto.getUserName() + "님 가입을 환영합니다. 이메일을 받지 못했을 경우에는 1588-1588로 문의 주세요.";
			ss.smsSend(dto.getUserPh1(), textMassage);
			e.printStackTrace();
		}
	}
