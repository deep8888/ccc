...
public class CommentDAO extends DataBaseInfo {
	final String COLUMNS = "COMMENT_NO, CUSER_ID, " + "REG_DATE, COMMENT_SUBJECT," + "COMMENT_CONTENT";
	...
	public void commentUpdate(CommentDTO dto) {
		conn = getConnection();
		sql = " update comment1 " + " set comment_subject = ? ," + "     comment_content = ? "
	...
	public CommentRepliesDTO commentCollection(String commentNo) {
		...
		conn = getConnection();
		sql = " select c.comment_no cno, c.cuser_id cid, " + "     c.COMMENT_SUBJECT csubject , "
				+ "     c.COMMENT_CONTENT cCONTENT , c.REG_DATE cdate," + "     r.REPLY_NO rno, r.RUSER_ID rid,"
				+ "     r.REG_DATE rdate , r.REPLY_CONTENT rcontent" + " from comment1 c left outer join reply r"
				+ " on c.comment_no = r.comment_no " + " where c.comment_no = ?";
		try {
			pstmt = conn.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
			pstmt.setString(1, commentNo);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				dto.setCommentContent(rs.getNString("cCONTENT"));
				dto.setCommentNo(rs.getLong("cno"));
				dto.setCommentSubject(rs.getNString("csubject"));
				dto.setCuserId(rs.getNString("cid"));
	...
	public List<CommentDTO> commentSelectAll(int page, int limit) {
		...
		sql = " select * " + " from ( select rownum rn, " + COLUMNS + "        from (select " + COLUMNS
				+ "              from comment1 " + "              order by COMMENT_NO  desc)) "
	...
	public void commentInsert(CommentDTO dto) {
		conn = getConnection();
		sql = "insert into comment1( " + COLUMNS + ")" + " values((" + subQuery + "),?,sysdate,?,? )";
		try {
			pstmt = conn.prepareStatement(sql);
